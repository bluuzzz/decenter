...
from models.dqdetr.DOCHG import get_dochg_loss
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            losses += get_dochg_loss(outputs['oc_map'], outputs['density_map'],targets, alpha = 0.05)
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        if args.save_results:
            ...
            for i, (tgt, res, outbbox) in enumerate(zip(targets, results, outputs['pred_boxes'])):
                ...
                _res_imgid = tgt['image_id'].expand(_res_bbox.size(0), -1) #add
                res_info = torch.cat((_res_bbox, _res_prob.unsqueeze(-1), _res_label.unsqueeze(-1),_res_imgid), 1)
