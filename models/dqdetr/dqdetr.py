...
class DQDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, dn_meta, counting_output, num_select, ccm_feature, oc_feature = self.transformer(srcs, masks, poss, targets, args_dn)
        ...
        out['oc_map'] = oc_feature
        out['density_map'] = ccm_feature
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # if 'density_weight' in outputs.keys() and outputs['density_weight'] is not None:
        #     loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=2, density_weights = outputs['density_weight']) * src_logits.shape[1]
        # else:
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        losses = {}
        # if 'density_weight' in outputs.keys() and outputs['density_weight'] is not None:
        #     losses['loss_bbox'] = (loss_bbox * outputs['density_weight'][idx]).sum() / num_boxes
        # else:
        ...
        if 'density_weight' in outputs.keys() and outputs['density_weight'] is not None:
            losses['loss_giou'] = (loss_giou * outputs['density_weight'][idx]).sum() / num_boxes
        else:
