...
from .DOCHG import ObjectCenterFeatureGenerator
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.OCFG = ObjectCenterFeatureGenerator()
    ...
    def forward(self, srcs, masks, pos_embeds, dn_targets, args_dn):
        ...
        oc_feature = self.OCFG(memory, spatial_shapes)
        
        oc_feature_softmax = F.softmax(oc_feature, dim=1)
        oc_ccm_feature = ccm_feature * (1 + oc_feature_softmax)

        

        multi_oc_ccm_feature = self.multiscale(oc_ccm_feature)
        
        cgfe_out = self.CGFE(multi_oc_ccm_feature, memory, spatial_shapes)
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, dn_meta, counting_output, num_select, ccm_feature, oc_feature
